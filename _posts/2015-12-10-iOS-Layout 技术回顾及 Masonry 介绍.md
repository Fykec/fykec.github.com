...
作为一个 iOS 开发者，大家都知道，UIKit 默认是 MVC 架构的，Model，View，和Controller ，随着这几年 App 开发的普及，这三部分每一个都有很大的发展。比如 Model 现在有很多 JSON-binding 像 Mantle，JSONModel，Controller所代表的控制层也有很多思潮像 MVVM， MVP大热，对 View 现在出来的 UI 控件更是数不胜数，让人眼花缭乱，在那么多变化中，其实也有些核心的思想是一直没有变的，比如今天要谈的 Layout 技术。

Layout 是大家经常接触的一个话题，但却是很少去关注的话题。因为用起来太简单了，都是改大小，改位置。没觉得需要再做些什么。 

不就是 setFrame 吗？
确实只是 setFrame，但也可以不只是setFrame。细看你也会发现一整片天地。

Layout 技术看上去简单，它却是整个 GUI 框架的基础，上层技术的设计，是必须依赖它才能实现。


##Code vs xib family

很早之前（有多早？没有 iOS 之前就有 nib 了） 关于 view的 Layout就有两种写法。
